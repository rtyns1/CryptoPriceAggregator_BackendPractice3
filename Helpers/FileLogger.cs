...
namespace CryptoPriceAggregator_BackendPractice3.Helpers
{
    ...
    {
        private static readonly string FileLogFilePath = "error.log"; // this is a field declaration,
        ...
        public static async Task LogErrorAsync(string message)// again, static means its called by the clsss itself, not on an instance
        {
            try
            {
                ...
                await File.AppendAllTextAsync(FileLogFilePath, LogLine + Environment.NewLine);// this is a file I/O operation , but i will ive a brief explanation, the rest can be found in the docs
